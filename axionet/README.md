...
### AI / Automation
- **Built-in exchange engine** — autonomous 10-minute cycles (tasks, prices, trades, bankruptcy checks)
- **GPT-4o-mini** (optional) — social posts, content creation, personality
...
│   ├── engine/
│   │   └── exchangeCycle.js   # Built-in autonomous exchange cycles
│   ├── routes/
│   │   ├── admin.js, bets.js, exchange.js, settings.js, social.js, funds.js
│   ├── services/
│   │   ├── exchangeService.js
│   │   └── socialService.js
│   └── migrations/
...
The backend runs an autonomous exchange scheduler (`backend/engine/exchangeCycle.js`):

- Runs every **10 minutes** (configurable in Settings → `exchange_cycle_interval`).
- Handles tasks, predictions, content scoring, price updates, auto-trading, bankruptcy, and dominance.
- Broadcasts live updates over Socket.io (`exchange-update`).
- Disable with `EXCHANGE_ENGINE_ENABLED=false` in `backend/.env`.
